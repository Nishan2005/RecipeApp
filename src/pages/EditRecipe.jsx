...
function EditRecipe() {
  ...
  useEffect(() => {
    const fetchRecipeData = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get(`https://localhost:7043/api/Recipe/filterId?Id=${id}`);
        ...
        if (recipe.instructions && typeof recipe.instructions === 'string') {
            try {
              let parsedInstructions = JSON.parse(recipe.instructions);
              if (Array.isArray(parsedInstructions)) {
                // Filter out null, undefined, or empty string values
                parsedInstructions = parsedInstructions.filter(instruction => 
                  instruction !== null && 
                  instruction !== undefined && 
                  instruction !== ''
                );
                
                // Only set state if we have valid instructions
                if (parsedInstructions.length > 0) {
                  setInstructions(parsedInstructions);
                } else {
                  // Set a default empty instruction if all were filtered out
                  setInstructions(['']);
                }
              }
            } catch (e) {
  ...
  const handlePhotoUpload = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPhotoUploaded(true);
      setPhotoUrl(''); // Clear the existing URL as we're uploading a new file
    }
  };

  const handleInputChange = (e) => {
    const { id, value } = e.target;
    let fieldName = id.replace('recipe-', '').replace('-', '');
    
    // Match field names to state properties
    switch (id) {
      case 'recipe-title':
        fieldName = 'title';
        break;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://localhost:7043/api/Recipe/editRecipe', formData, {
        headers: {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-amber-50 flex items-center justify-center">
        <div className="text-xl">Loading recipe data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-amber-50 text-gray-800 font-sans">
      <div className="container max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-3xl text-amber-800 font-bold text-center mb-6">Edit Recipe</h1>
